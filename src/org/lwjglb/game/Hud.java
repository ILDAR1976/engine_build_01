...
public class Hud {
    ...
    public void init(EWindow EWindow) throws Exception {
        this.vg = EWindow.getOptions().antialiasing ? nvgCreate(NVG_ANTIALIAS | NVG_STENCIL_STROKES) : nvgCreate(NVG_STENCIL_STROKES);
        if (this.vg == NULL) {
    ...
    public void render(EWindow EWindow) {
        nvgBeginFrame(vg, EWindow.getWidth(), EWindow.getHeight(), 1);

        // Upper ribbon
        nvgBeginPath(vg);
        nvgRect(vg, 0, EWindow.getHeight() - 100, EWindow.getWidth(), 50);
        nvgFillColor(vg, rgba(0x23, 0xa1, 0xf1, 200, colour));
        nvgFill(vg);

        // Lower ribbon
        nvgBeginPath(vg);
        nvgRect(vg, 0, EWindow.getHeight() - 50, EWindow.getWidth(), 10);
        nvgFillColor(vg, rgba(0xc1, 0xe3, 0xf9, 200, colour));
        nvgFill(vg);

        glfwGetCursorPos(EWindow.getEWindowHandle(), posx, posy);
        int xcenter = 50;
        int ycenter = EWindow.getHeight() - 75;
        int radius = 20;
        ...
        nvgText(vg, 50, EWindow.getHeight() - 87, String.format("%02d", counter));
        ...
        nvgText(vg, EWindow.getWidth() - 150, EWindow.getHeight() - 95, dateFormat.format(new Date()));

        nvgEndFrame(vg);

        // Restore state
        EWindow.restoreState();
    }
